...
import { ArrowForward, CloseRounded, Email, LockRounded, MailOutlineRounded, Password, ReportGmailerrorredRounded, VisibilityOffRounded, VisibilityRounded } from "@mui/icons-material"
...
export default function ForgotPasswordGame() {
    ...
    const forgotPassword = async () => {
        ...
        if (emailAddress == "") {
            setError(true);
            setToastItem({ toastType: 'error', toastMessage: 'Email field must not be empty!' });
            setLoading(false);
            setTimeout(() => {
        ...
        if (!emailRegex.test(emailAddress)) {
            setToastItem({ toastType: 'error', toastMessage: 'Enter a valid email address' });
            setLoading(false);
            setTimeout(() => {
                setToastItem({ toastType: '', toastMessage: '' });
            }, 2000)
        ...
        try {
            ...
            setCountDown(60);
            setLoading(false);
            setToastItem({ toastType: 'success', toastMessage: `Password reset mail sent to ${emailAddress}` });
            setTimeout(() => {
                setToastItem({ toastType: '', toastMessage: "" });
            }, 2000)
        } catch (error: any) {
            ...
            <div className="z-[20] p-2 md:p-0 w-[371px]">
                ...
                <div className="items-center mb-5">
                    ...
                    {canViewGoBackMsg ?
                        ...
                        <p className="text-center text-[14px] text-vivd-lime-green-10">Enter your email to retrieve your password</p>
                ...
                {!sendEmail &&
                    <div className="items-center justify-center">
                        <CustomInput
                            ...
                            type={"text"}
                            addOnStart={<Email color="inherit" />}
                        />

                        <div className={`${emailAddress.length >= 0 ? "my-5" : "blur-[2px] my-5"}`}>
                            <button disabled={emailAddress.length == 0} onClick={() => forgotPassword()} className="navbar-auth-btn buttonTracker  w-full">{loading ? <CircularProgress size={14} color="inherit" /> : 'Send OTP'}</button>
                        </div>
                ...
                {sendEmail && <>
                    {!canViewGoBackMsg && <div className="items-center justify-center">
                        <div className="flex flex-row justify-between items-center">
                            <p style={{ cursor: 'pointer' }} className=" text-[#A5E314] text-center my-5 pe-5">
                                Resend Link in <span className="text-[#52594B]">{countDown}s</span>
                            </p>
                            <p onClick={() => setSendEmail(false)} style={{ cursor: 'pointer' }} className=" text-[#A5E314] text-center my-5 pe-5">
                                Change Email
                            </p>
                        </div>
                        ...
                        {password.length == 0 &&
                            <div className={`${countDown <= 0 ? "my-5" : "blur-[2px] my-5"}`}>
                                <button disabled={countDown > 0} onClick={() => resendOtp()} className=" buttonTracker py-3 border w-full component_btn_transparent border-vivd-lime-green rounded-xl text-vivd-lime-green-10">{loading ? <CircularProgress size={14} color="inherit" /> : 'Resend OTP'}</button>
                            </div>
                        }
                        <div className={`${errors.length == 0 ? "my-5" : "blur-[2px] my-5"}`}>
                            <button disabled={password.length <= 0} onClick={() => UpdatePassword()} className="navbar-auth-btn buttonTracker  w-full">{loading ? <CircularProgress size={14} color="inherit" /> : 'Update Password'}</button>
                        </div>
